...
class MarigoldTrainer:
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                rgb = batch["audio_spec"].to(device)
                audio_wave= batch["audio_wave"].to(device)
                # rgb torch.Size([8, 2, 128, 128])

                # depth_gt_for_latent = batch[self.gt_depth_type].to(device)
                depth_gt_for_latent = batch["depth"].to(device)
              
            #  depth_gt_for_latent torch.Size([8, 1, 128, 128])
                ...
                if self.gt_mask_type is not None:
                    ...
                    # 从深度图直接生成有效掩码 (深度值大于0的像素为有效)
                    valid_mask_for_latent = (depth_gt_for_latent > 0).bool()
                    invalid_mask = ~valid_mask_for_latent
                    valid_mask_down = ~torch.max_pool2d(
                        invalid_mask.float(), 8, 8
                    ).bool()
                ...
                with torch.no_grad():
                    # vae_model_path = "/home/yinjun/project/Marigold-main/vae_mult/VAE_UNet_MultiASFF.pth"
                    # vae_model = load_pretrained_vae_unet( vae_model_path)
                    # vae_model.to(device)
                    # Encode image
                     #变为3通道
                    # rgb = torch.cat([rgb, rgb[:, :1, :, :]], dim=1)
               
                    # rgb_latent = self.model.encode_spec(rgb) 
                    rgb_latent = self.model.encode_rgb(rgb)
                
          
                 
                    # rgb_latent = encode_with_pretrained_vae_unet( vae_model, rgb)
                
                    # depth_gt_for_latent = depth_gt_for_latent.repeat(1, 2, 1, 1)
                    # 测试解码器
                    # gt_depth_latent = encode_with_pretrained_vae_unet( vae_model, depth_gt_for_latent)
      


                    # rgbLatent: torch.Size([2, 4, 16, 16])
                    # Encode GT depth
                    gt_depth_latent = self.encode_depth(
                        depth_gt_for_latent
                    )  
                    # gt_depth_latent torch.Size([2, 4, 16, 16])
                ...
                if hasattr(self.model, 'audio_embed_model') and self.model.audio_embed_model is not None:
                
                    with torch.no_grad():
                        # 使用音频嵌入模型处理波形数据
                        self.model.audio_embed_model = self.model.audio_embed_model.to(device)
                        audio_embed = self.model.audio_embed_model.echo_encoder(audio_wave)
                        
                        # 根据需要调整形状，确保与模型期望的维度一致
                        if audio_embed.dim() == 2:  # [B, embed_dim]
                            audio_embed = audio_embed.unsqueeze(1)  # [B, 1, embed_dim]
                        
                        # 添加投影层进行维度调整 (768 -> 1024)
                        if not hasattr(self.model, 'audio_projection'):
                            self.model.audio_projection = torch.nn.Sequential(
                        ...
                        # 进行维度投影
                        audio_embed = self.model.audio_projection(audio_embed)  # [B, 1, 1024]
                        
                        # 将序列长度扩展到与CLIP文本编码器相同 (77个token)
                        audio_embed = audio_embed.repeat(1, 77, 1)  # [B, 77, 1024]

                        # 使用音频嵌入作为条件
                        text_embed = audio_embed.to(device)
                else:
                    print("没有音频嵌入模型，回退到使用空文本嵌入")
                    # 如果没有音频嵌入模型，回退到使用空文本嵌入
                ...
                with torch.no_grad():
                # 使用预测的潜在向量重建深度图
                 reconstructed_depth = self.model.decode_depth(model_pred)
                ...
                # 使用sigmoid将权重限制在[0, 0.1]范围内
                # recon_weight = 0.3 * (1 - timesteps.float() / self.scheduler_timesteps)
                max_weight = 0.2
                T = self.scheduler_timesteps
                recon_weight = max_weight * torch.sigmoid(-3 * (timesteps / T - 0.5))
                use_recon = (timesteps < 200)
                if use_recon.any():
                    loss = latent_loss.mean() + recon_weight[use_recon].mean() * recon_loss
                else:
                    loss = latent_loss.mean()
                # loss = latent_loss.mean() +recon_weight.mean() * recon_loss
                # print(latent_loss.mean(), recon_weight.mean(), recon_loss)
    ...
    def save_checkpoint(self, ckpt_name, save_train_state):
        ...
        #保存线性层投影
        if hasattr(self.model, 'audio_projection'):
            audio_proj_path = os.path.join(ckpt_dir, "audio_projection.pth")
            torch.save(self.model.audio_projection.state_dict(), audio_proj_path)
            logging.info(f"Audio projection layer saved to: {audio_proj_path}")
            
        if hasattr(self.model, 'audio_embed_model') and self.model.audio_embed_model is not None:
            audio_model_path = os.path.join(ckpt_dir, "audio_embed_model.pth")
            torch.save(self.model.audio_embed_model.state_dict(), audio_model_path)
            logging.info(f"Audio embedding model saved to: {audio_model_path}")
